  ...
  <div class="info">
    ...
    <div class="car-info">
      <div class="section">
        ...
        <div class="item">当前状态: {{car.bindTime && car.status ? computedCarStatu(car.status, car.bindTime) : '---'}}<span v-if="car.bindTime && car.status"><zx-icon customClass="icon-power error" v-if="computedCarStatu(car.status, car.bindTime).includes('超时')" type="zx-chaoshigaojing1"></zx-icon><zx-icon customClass="icon-power error" v-if="computedCarStatu(car.status, car.bindTime).includes('告警')" type="zx-alarm"></zx-icon></span></div>
        ...
        <div class="item">当前环节: {{car.startTime ? computedCarNode(car.node) + ' -- ' + $moment(car.startTime).toNow(true) : computedCarNode(car.node)}}</div>
      ...
      <div class="section">
        <el-button @click="handleClick(type, car.locatorId)" round type="primary">{{type === 'repair' ? '结束维修' : '开始维修'}}</el-button>
      </div>
    </div>
  </div>
</template>
